...
# Example 4: liveness property
# all executions must end with x
#In Declare: End(x)
def example4():
    aut = factory.generateAutomatonFromTemplate("End", ["x"])
    # aut.visualizeAutomaton("All executions must end with x")
    
    compliant_input = aut.runTrace(["x", "!x", "!x", "x"])
    print("Compliance: ", compliant_input.traceAcceptance)
    print("Output Trace: ", compliant_input.outputTrace)
    print("Suppressions: ", compliant_input.suppressions,
        " Insertions: ", compliant_input.insertions,
    ...
    editted_input = aut.runTrace(["x", "!x", "!x", "!x"])
    print("Compliance: ", editted_input.traceAcceptance)
    print("Output Trace: ", editted_input.outputTrace)
    print("Suppressions: ", editted_input.suppressions,
        " Insertions: ", editted_input.insertions,
...
# Example 5: Generate automata from a Declare model file
def example5():
...
# Example 6: Enforcing complex policies with multiple, simple, security automata
...
def example6():
    ...
    inputAEdited = ["Show", "Board", "Show","Board"]
